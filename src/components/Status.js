...
class Status extends React.Component {
        ...
        if (res === null){
          ...
          toast.error('Connection Error', {
            ...
            if(res.status !==200){
              ...
              let msg = this.state.editing===0?"Status Added":"Status Updated";
              toast.success(msg, {
                position: "top-right",
                autoClose: 2000,
                hideProgressBar: false,
  ...
  if(this.state.name){
    ...
    axios({
      method: "post",
      url: this.state.editing===0?`${DB_Link}status_create.php`:`${DB_Link}status_update.php`,
      data: JSON.stringify(_data),
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json"
  ...
  render() {
    return (
     ...
     <MDBDataTable
          ...
          <MDBContainer>
          <form onSubmit={(e)=>{this.addStatus(e)}}>

      <MDBModal isOpen={this.state.modal}  size='lg' toggle={this.toggle} >
        <MDBModalHeader toggle={this.toggle}>Add New Status</MDBModalHeader>
        <MDBModalBody>
        ...
        <MDBModalFooter>
        <button type="submit"  onClick={(e)=>{this.addStatus(e)}} className="login100-form-btn submitButn">
                                Submit
                            </button>
                                    </MDBModalFooter>
      </MDBModal>
