...
class Types extends React.Component {
  state = { 
    data: {}, loader: 'flex', tbl: 'none' ,mandatoryCheck:false,  modal: false,role:'',id:'',name:"",editing:0,
  }
  ...
  addTypes=(e)=> {
    ...
    axios({
      method: "post",
      url: this.state.editing===0?`${DB_Link}types_create.php`:`${DB_Link}types_update.php`,
      data: JSON.stringify(_data),
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json"
  ...
  render() {
    return (
     ...
     <MDBDataTable
          ...
          <MDBContainer>
          <form onSubmit={(e)=>{this.addTypes(e)}}>

      <MDBModal isOpen={this.state.modal}  size='lg' toggle={this.toggle} >
        <MDBModalHeader toggle={this.toggle}>Add New Type</MDBModalHeader>
        <MDBModalBody>
        ...
        <MDBModalFooter>
        <button type="submit"  onClick={(e)=>{this.addTypes(e)}} className="login100-form-btn submitButn">
                                Submit
                            </button>
                                    </MDBModalFooter>
      </MDBModal>
